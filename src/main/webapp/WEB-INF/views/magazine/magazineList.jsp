	...
	<div class="search-container">
		...
		<div id="search-region" class="search-type">
			<form action="<%=request.getContextPath()%>/magazine/magazineFinder">
				<input type="hidden" name="searchType" value="region" />
				<input type="text" name="searchWord" size="25" placeholder="검색할 지역을 입력하세요." 
					value="<%= "region".equals(searchType) ? searchWord : ""%>" />
				<button type="submit" id= "btn" class="search-btn">검색</button>
			</form>
		</div>
		<!-- <div id="search-writer" class="search-type">
			<form action="<%= request.getContextPath() %>/magazine/magazineFinder">
				...
				<input type="text" name="searchWord" size="25" placeholder="검색할 아이디를 입력하세요."
					value="<%= "userId".equals(searchType) ? searchWord : "" %>" />
